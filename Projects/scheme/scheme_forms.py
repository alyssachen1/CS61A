...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if clause.first == 'else':
            ...
            test = scheme_eval(clause.first, env) #value of evaluating pred
        if is_scheme_true(test): #if pred is true
            # BEGIN PROBLEM 13
            if clause.rest is nil: #and if subexpression of pred is empty
                return test #return pred value
            return eval_all(clause.rest, env) #return this if there are multiple expressions
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    validate_formals(names) #checks only one name bound in local frame
    # END PROBLEM 14
