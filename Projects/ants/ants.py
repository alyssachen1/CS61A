...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        current = self.place
        place_holder = 0
        while current.entrance: #while current entrance exists
            if not current.is_hive and current.bees: #if current place is not hive and has bees
                if self.lower_bound <= place_holder <= self.upper_bound:
...
class WallAnt(Ant):
    ...
    def __init__(self, health=4):
        """Create an Ant with HEALTH quantity"""
        super().__init__(health)
# END Problem 6
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt selects random Bee and deals damage to Bee by eating it whole"""
    ...
    def action(self, gamestate):
        ...
        elif self.turns_to_chew == 0 and self.place.bees: #if it is not chewing and there is a bee in its plave
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while current.exit != None:
            ...
            if current.ant != None:
                if current.ant.is_container and current.ant.ant_contained != None:
